- # FPGA 实时 Sobel 边缘检测系统
  ...
  - 1280×720 实时摄像头采集
  - 硬件 Sobel 边缘检测，参数化阈值（默认 100，可通过参数修改）
  - 按键实时切换：原始彩色画面 ↔ 边缘黑白画面
  - 手写 FWFT 异步 FIFO，实现跨时钟域（PCLK → ACLK）可靠传输
  - 标准 AXI-Stream 输出，直接对接 AXI VDMA
  - 全模块 IP 化封装，支持在 Block Design 中一键调用

  ## 系统框图

  > 注：若无法显示图片，请打开同目录下的 `block_diagram.png` 查看。
  ...
  **时钟域划分**：  
  ...
  > 数据来源：Vivado 2017.4 综合实现后 `report_utilization`。

  | 资源            | 用量 | 比例 |
  | --------------- | ---- | ---- |
  | Slice LUTs      | xxx  | x%   |
  | Slice Registers | xxx  | x%   |
  | Block RAM Tile  | xxx  | x%   |
  | DSP48E1         | xxx  | x%   |

  **获取资源数据的方法**：  
  在 Vivado 中依次点击 `Reports → Report Utilization`，将表格数据填入上面位置。
